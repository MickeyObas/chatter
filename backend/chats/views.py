...
from .serializers import (ChatDisplaySerializer, ChatSerializer,
...
def chat_detail(request, pk):
    try:
        ...
        return Response(
            {"message": "Chat does not exist"}, status=status.HTTP_404_NOT_FOUND
        )
...
def chat_detail_by_contact(request, pk):
    try:
        ...
        chat, created = Chat.objects.get_or_create(
            owner=request.user, user=contact.contact_user
        )
...
def set_last_read_message(request, pk):
    try:
        chat = Chat.objects.get(id=pk, owner=request.user)
...
def set_unread_messages_to_read(request, chat_id):
    try:
        ...
        return Response(
            {"error": "An error occured in changing the read status of messages"},
...
def group_chat_detail(request, pk):
    try:
        ...
        serializer = GroupChatSerializer(
            group_chat, context={"user_id": request.user.id}
        )
        return Response(serializer.data)
...
def group_chat_list_or_create(request):
    ...
    if request.method == "GET":
        ...
        serializer = GroupChatDisplaySerializer(
            group_chats, many=True, context={"user_id": user.id}
        )
        return Response(serializer.data)
    else:
        data = request.data
        ...
        serializer = GroupChatCreateSerializer(
            data=group_chat_data, context={"request": request}
        )
        ...
        if serializer.is_valid():
            ...
            for user_member in new_group_chat.members.all():
                for other_member in new_group_chat.members.exclude(id=user_member.id):
            ...
            serializer = GroupChatDisplaySerializer(
                new_group_chat, context={"user_id": user.id}
            )
            return Response(serializer.data)
        else:
...
def add_contact_to_group(request, groupchat_id):
    try:
        ...
        if not Contact.objects.filter(
            user=request.user, contact_user=user_to_add
        ).exists():
            return Response(
                {
        ...
        for member in groupchat.members.exclude(id=user_to_add.id):
            UserGroupContactColorMap.objects.create(
                user=user_to_add, group=groupchat, contact_user=member
            )

            UserGroupContactColorMap.objects.create(
                user=member, group=groupchat, contact_user=user_to_add
            )
