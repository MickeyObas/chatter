...
from chats.serializers import (ChatDisplaySerializer, ChatSerializer,
                               GroupChatDisplaySerializer, GroupChatSerializer)
from messaging.models import GroupChatMessage, Message
from messaging.serializers import (CreateGroupChatMessageSerializer,
...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        recipient = await database_sync_to_async(CustomUser.objects.get)(
            id=recipient_id
        )

        # Get (or create) their individual chats
        owner_chat, owner_chat_created = await database_sync_to_async(
            Chat.objects.get_or_create
        )(owner=sender, user=recipient)
        recipient_chat, recipient_chat_created = await database_sync_to_async(
            Chat.objects.get_or_create
        )(owner=recipient, user=sender)
        ...
        if await database_sync_to_async(owner_serializer.is_valid)(
            raise_exception=True
        ) and await database_sync_to_async(recipient_serializer.is_valid)(
            raise_exception=True
        ):
            new_message = await database_sync_to_async(owner_serializer.save)()
            recipient_new_message = await database_sync_to_async(
...
class NotificationConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        await self.channel_layer.group_send(
            "online_users", {"type": "user.online", "user_id": self.user_id}
        )
    ...
    async def disconnect(self, close_code):
        ...
        await self.channel_layer.group_send(
            "online_users", {"type": "user.offline", "user_id": self.user_id}
        )
        ...
        try:
            while True:
                ...
                online_users = online_users_list = [
                    int(x) for x in list(redis_client.smembers("online_users"))
                ]
...
class GroupChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        await self.channel_layer.group_add(self.group_chat_name, self.channel_name)
    ...
    async def receive(self, text_data=None, bytes_data=None):
        data = json.loads(text_data)
        groupchat = await database_sync_to_async(GroupChat.objects.get)(
            id=data["groupchat_id"]
        )
        sender = await database_sync_to_async(CustomUser.objects.get)(
            id=data["sender_id"]
        )
        content = data["content"]
        ...
        group_chat_message_serializer = CreateGroupChatMessageSerializer(
            data=message_data
        )

        # Create message in DB
        if await database_sync_to_async(group_chat_message_serializer.is_valid)(
            raise_exception=True
        ):
            new_groupchat_message = await database_sync_to_async(
                group_chat_message_serializer.save
            )()
            ...
            # Send to each user
            groupchat_data = await get_group_chat_detail(
                new_groupchat_message.groupchat.id, self.user_id
            )
