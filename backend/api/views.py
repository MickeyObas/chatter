...
def login(request):
    ...
    if user is not None:
        ...
        if not user.is_verified:
            return Response(
                {
                    "error": "Email is not verified. Please check your email to complete registration."
                },
...
def email_confirm(request, token):
    ...
    return Response(
        {"message": "Email confirmed. Redirecting to Login"}, status=status.HTTP_200_OK
    )
    ...
    if response.status_code == 200:
        ...
        try:
            ...
            idinfo = id_token.verify_oauth2_token(
                response_data["id_token"], google_requests.Request(), CLIENT_ID
            )
            ...
            print(
                f"User {email} - {name} has been verified and confirmed as a goolgle user."
            )
